...
Geometry3D MeshLoader::LoadModel(const std::string& filepath) const
{
...
FlatMaterialPars MeshLoader::GetFlatMaterialParameters(aiMaterial* OtherMat) const
{
...
void MeshLoader::AssignMaterials(std::vector<MaterialInfo>& Materials, 
...
void MeshLoader::AssignMeshes(std::vector<MeshData>& MyMeshes, aiMesh** OtherMeshes, uint32_t count) const
{
	MyMeshes.clear();
	...
	for (uint32_t i = 0; i < count; i++)
	{
		...
		MyMesh.AssignFaces((aiFace*) OtherMesh->mFaces, OtherMesh->mNumFaces);

		MyMesh.MaterialRef = static_cast<size_t>(OtherMesh->mMaterialIndex);
	}
}

void MeshLoader::FillTextures(MaterialInfo& MyMat, aiMaterial* OtherMat) const
{
...
aiTextureType GetTextureType(TextureType type)
{
	switch (type)
	{
		...
		case TextureType::eUnkown:			       return aiTextureType_UNKNOWN;
