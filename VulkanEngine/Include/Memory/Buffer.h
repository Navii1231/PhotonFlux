...
void Buffer<bool>::SetBuf(Iter Begin, Iter End, size_t Offset)
{
	...
	Byte* Memory = MapMemory<Byte>(Count * ElemSize, Offset * ElemSize);
	std::memcpy(Memory, &(*Begin), Count * ElemSize);
	UnmapMemory();
...
template <typename Type>
void Buffer<bool>::FetchMemory(Type* Begin, Type* End, size_t Offset)
{
	if (Begin == End)
		return;
	...
	std::memcpy(Begin, Memory, Count * sizeof(Type));
	UnmapMemory();
}
...
void Buffer<bool>::UnmapMemory() const
{
	mChunk.Device->unmapMemory(mChunk.BufferHandles->Memory);
...
void Buffer<bool>::InsertMemoryBarrier(vk::CommandBuffer commandBuffer, const MemoryBarrierInfo& pipelineBarrierInfo)
{
	vk::BufferMemoryBarrier barrier;
	barrier.setBuffer(mChunk.BufferHandles->Handle);
	barrier.setSrcAccessMask(pipelineBarrierInfo.SrcAccessMasks);
...
void Buffer<bool>::TransferOwnership(uint32_t DstQueueFamilyIndex) const
{
	uint32_t& SrcOwner = mChunk.BufferHandles->Config.ResourceOwner;
...
void Buffer<bool>::TransferOwnership(vk::QueueFlagBits optimizedCaps) const
{
	uint32_t DstQueueFamily = mProcessHandler
		.GetQueueManager().FindOptimalQueueFamilyIndex(optimizedCaps);
	TransferOwnership(DstQueueFamily);
...
void Buffer<bool>::Reserve(size_t NewCap)
{
	if (NewCap > mChunk.BufferHandles->Config.ElemCount)
		ScaleCapacityWithoutLoss(NewCap);
}

void Buffer<bool>::Resize(size_t NewSize)
{
	Reserve(NewSize);
	mChunk.BufferHandles->ElemCount = NewSize;
}

void Buffer<bool>::ScaleCapacity(size_t NewSize)
{
	mChunk.BufferHandles->Config.ElemCount = NewSize;
	Core::Buffer NewBuffer = Core::Utils::CreateBuffer(mChunk.BufferHandles->Config);
...
void Buffer<bool>::ScaleCapacityWithoutLoss(size_t NewSize)
{
	mChunk.BufferHandles->Config.ElemCount = NewSize;
	Core::Buffer NewBuffer = Core::Utils::CreateBuffer(mChunk.BufferHandles->Config);
...
template <typename T>
Buffer<bool>& operator <<(Buffer<bool>& vkBuf, const std::vector<T>& cpuBuf)
{
	vkBuf.AppendBuf(cpuBuf.begin(), cpuBuf.end());
	return vkBuf;
}
...
template <typename T>
Buffer<bool>& operator <<(Buffer<bool>& vkBuf, std::initializer_list<T> cpuBuf)
{
	vkBuf.AppendBuf(cpuBuf.begin(), cpuBuf.end());
	return vkBuf;
}
...
template <typename T, size_t Size>
Buffer<bool>& operator <<(Buffer<bool>& vkBuf, const std::array<T, Size>& cpuBuf)
{
	vkBuf.AppendBuf(cpuBuf.begin(), cpuBuf.end());
	return vkBuf;
}
...
template <typename T>
Buffer<bool>& operator >>(Buffer<bool>& vkBuf, std::vector<T>& cpuBuf)
{
	size_t offset = cpuBuf.size();
	cpuBuf.resize(cpuBuf.size() + vkBuf.GetSize());
...
template <typename T>
Buffer<bool>& operator <<(Buffer<bool>& vkBuf, const T& cpuVal)
{
	vkBuf.AppendBuf(&cpuVal, &cpuVal + 1);
	return vkBuf;
}

void Buffer<bool>::PerformCopyTaskGPU(Core::Buffer& DstBuffer, const Core::Buffer& SrcBuffer,
	const vk::ArrayProxy<vk::BufferCopy>& CopyRegions)
{
	if (CopyRegions.front().size == 0)
		return;
...
inline void Buffer<bool>::ReleaseBuffer(uint32_t dstIndex, vk::Semaphore bufferReleased) const
{
	uint32_t OwnerIndex = mChunk.BufferHandles->Config.ResourceOwner;
...
inline void Buffer<bool>::AcquireBuffer(uint32_t acquiringFamily, vk::Semaphore bufferReleased) const
{
	uint32_t OwnerIndex = mChunk.BufferHandles->Config.ResourceOwner;
	uint32_t DstIndex = acquiringFamily;
